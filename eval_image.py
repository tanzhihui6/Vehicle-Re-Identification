...
parser.add_argument('--queryFeat', default='./results/veri/resnet101/queryFeat.npy', type=str, metavar='DIR', help='path to query feature')
parser.add_argument('--galleryFeat', default='./results/veri/resnet101/galleryFeat.npy', type=str, metavar='DIR', help='path to gallery feature')
...
def main():
    ...
    np.savetxt('query_feat_mat.txt', query_feat_mat, fmt='%.4f')
    np.savetxt('gallery_feat_mat.txt', gallery_feat_mat, fmt='%.4f')
    ...
    for gallery_line in gallery_lines:
        ...
        gallery_pids.append(int(line.split('_')[0]))
        gallery_camids.append(int(line.split('_')[1].split('c')[1]))

    distmat = np.zeros((query_feat_mat.shape[0], gallery_feat_mat.shape[0]))
    distmat = np.sqrt(getNormMatrix(query_feat_mat, gallery_feat_mat.shape[0]).T + getNormMatrix(gallery_feat_mat, query_feat_mat.shape[0]) - 2 * np.dot(query_feat_mat, gallery_feat_mat.T))
    print('Dist mat: ', distmat.shape)
    #np.savetxt('distmat.txt', distmat, fmt='%.4f')

    cmc, mAP = eval_func(distmat, query_pids, query_camids, gallery_pids, gallery_camids, max_rank=100)
